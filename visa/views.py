...
def redirect_to_otp(request):
    if request.method == 'POST':
        ...
        if signup_type == 'consultant':
                ...
                if company_name and phone and email and full_name and district_id and thana_id and address and password and confirm_password:
                    if password == confirm_password:
                        consultant_phone = Users.objects.filter(phone=phone).first()
                        user_phone = CustomUser.objects.filter(phone=phone, user_type=1).first()
                        consultant_email = Users.objects.filter(email=email).first()
                        user_email = CustomUser.objects.filter(email=email, user_type=1).first()
                        valid_phone_number = re.match(r'^(013|014|015|016|017|018|019)\d{8}$', phone)
                                ...
                                if valid_phone_number:
                                    ...
                                    try:
                                        ...
                                        if response.status_code == 200:
                                            return JsonResponse({'success': True, 'redirect_url': '/otp_verification_signup/', 'expiration_time': expiration_time})
                                        else:
                                            return JsonResponse({'errors': f'Server returned status code {response.status_code}. Please try again later.'})

                                    except Timeout:
                                        return JsonResponse({'errors': 'Request timed out. Please try again later.'})

                                    except ConnectionError:
                                        return JsonResponse({'errors': 'Failed to establish connection to the server. Please check your internet connection and try again later.'})
                ...
                if full_name and email and phone and user_type and gender and address and district_id and thana_id and countries and password and confirm_password:
                    if password == confirm_password:
                        consultant_phone = Students.objects.filter(phone=phone).first()
                        consultant_email = Students.objects.filter(email=email).first()
                        valid_phone_number = re.match(r'^(013|019|018|014|015|016|017)\d{8}$', phone)
                            ...
                            if consultant_email is None:
                                if valid_phone_number:
                                    ...
                                    try:
                                        ...
                                        if response.status_code == 200:
                                            return JsonResponse({'success': True, 'redirect_url': '/otp_verification_signup_student/', 'expiration_time': expiration_time})
                                        else:
                                            return JsonResponse({'errors': f'Server returned status code {response.status_code}. Please try again later.'})
...
def otp_verification_signup(request):
    ...
    return render(request, 'user_login/otp_verification_signup.html', {'expiration_time': time_remaining, 'phone': phone_number,'page_name':page_name,         'page_description': page_description,
         'page_keywords': page_keywords})
...
def otp_verification_signup_student(request):
    ...
    return render(request, 'user_login/otp_verification_signup_student.html', {'expiration_time': time_remaining, 'phone': phone_number,'page_name':page_name,'page_description': page_description,
         'page_keywords': page_keywords})
...
def change_number(request):
    if request.method == 'GET':
        ...
        if phone:
            ...
            consultant_user = CustomUser.objects.filter(phone=phone, user_type=1).first()
            ...
            if user is None:
                if valid_phone_number:
                    ...
                    request.session['temp_user_data']['otp'] = otp
                    request.session['temp_user_data']['expiration_time'] = expiration_time
                    ...
                    if response.status_code == 200:
                        return JsonResponse({'success': True, 'redirect_url': '/otp_verification_signup_student/', 'expiration_time': expiration_time, 'changed_phone': changed_phone})
            else:
...
def forgot_password_otp_verification(request):
    ...
    return render(request, 'user_login/forgot_password_otp_verification.html', {'expiration_time': time_remaining,'page_name': page_name,         'page_description': page_description,
         'page_keywords': page_keywords})
...
def change_forgotten_password(request):
    ...
    return render(request, 'user_login/change_forgotten_password.html',{'page_name': page_name,  'page_description': page_description,
         'page_keywords': page_keywords})
...
def forgot_password_phone_or_email_student(request):
    ...
    return render(request, 'user_login/forgot_password_phone_or_email_student.html',{'page_name': page_name,'page_description': page_description,
         'page_keywords': page_keywords})
...
def forgot_password_otp_verification_student(request):
    ...
    return render(request, 'user_login/forgot_password_otp_verification_student.html', {'expiration_time': time_remaining,'page_name': page_name,         'page_description': page_description,
         'page_keywords': page_keywords})
...
def change_forgotten_password_student(request):
    ...
    # else:
    return render(request, 'user_login/change_forgotten_password_student.html',{'page_name': page_name,'page_description': page_description,
         'page_keywords': page_keywords})
...
def login_user(request):
    ...
    if request.method == 'POST':
        ...
        if identifier is not None and password is not None:
            if '@' in identifier:  # Assuming email contains '@'
                ...
                root_user = EmailBackend().authenticate(request, email=identifier, password=password, user_type=0)
                consultant_user = EmailBackend().authenticate(request, email=identifier, password=password, user_type=1)
                ...
                root_user = EmailBackend().authenticate(request, phone=identifier, password=password, user_type=0)
                consultant_user = EmailBackend().authenticate(request, phone=identifier, password=password, user_type=1)
    ...
    return render(request, 'user_login/login_user.html',{'page_name': page_name, 'page_description': page_description,
         'page_keywords': page_keywords})
...
def login_student(request):
    ...
    if request.method == 'POST':
        ...
        identifier = request.POST.get('identifier')  # Use 'identifier' for both email and phone
        password = request.POST.get('password')
        ...
        if '@' in identifier:  # Assuming email contains '@'
            user = EmailBackend().authenticate(request, email=identifier, password=password, user_type=2)
        else:
            user = EmailBackend().authenticate(request, phone=identifier, password=password, user_type=2)
...
def contact(request):
    ...
    if request.method == 'POST':
        ...
        if cap_json.get('success', False):
            try:
                ...
                recipient_email = settings.EMAIL_HOST_USER  # Send email to configured email address
    ...
    return render(request, 'contact.html', {'addresses': addresses, 'category_choices': category_choices, 'consultant_maps': consultant_maps, 'page_name': page_name,'page_description': page_description,
         'page_keywords': page_keywords})
...
def process_explanation(request):
    ...
    return render(request, 'process_explenation.html', {'explanations': explanations,'page_name': page_name,'page_description': page_description,'page_keywords': page_keywords})
...
def consultant_list(request):
    ...
    consultants = Users.objects.filter(id__in=users.values('id'), active_status__in=[1, 3, 4, 5]).order_by('id')
    fav_consultant_ids = []
    ...
    consultant_details = ConsultantDetails.objects.filter(consultant_id__in=consultants).order_by('consultant_id')
    countries = Countries.objects.all()
    ...
    consultants_with_details = [{'consultant': consultant, 'details': details} for consultant, details in
                                 zip(consultants, consultant_details)]
    ...
    if request.method == 'GET':
        ...
        queryset = Users.objects.filter(id__in=users.values('id'),  active_status__in=[1, 3, 4, 5])
        ...
        if search_input or rating_filter or country_filter or experience_filter:
            ...
            if rating_filter == 'high':
                ...
                queryset_user = Users.objects.filter(company_name__icontains=search_input,  active_status__in=[1, 3, 4, 5]).order_by('-rating')
            ...
            if experience_filter and experience_filter == 'high':
                if country_filter:
                    ...
                    ordering_conditions = [When(id=user_id, then=pos) for pos, user_id in enumerate(queryset, start=1)]
                    ...
                    print('queryset when the experience is high and country is there: ', queryset)
                ...
                if country_filter:
                    ...
                    consultant_details_ids = list(consultant_details.values_list('consultant_id', flat=True))
                    user_ordering_conditions = [When(id=user_id, then=pos) for pos, user_id in enumerate(consultant_details_ids, start=1)]
                    consultant_users = Users.objects.filter(id__in=consultant_details_ids).order_by(Case(*user_ordering_conditions, default=0))
            ...
            chunked_consultants = [consultants_with_details[i:i + 1] for i in range(0, len(consultants_with_details))]
            paginator = Paginator(consultants_with_details, 5)  # Show 5 consultants per page
            page = request.GET.get('page')
...
def consultant_list_autosearch(request):
    ...
    payload = []
    if search:
        objs = Users.objects.filter(company_name__icontains=search, active_status__in=[1, 3, 4, 5])
        for obj in objs:
...
def singel_consultant_details(request, consultant_id):
    consultant = get_object_or_404(Users, id=consultant_id, user_role=5)
    consultant_details = get_object_or_404(ConsultantDetails, consultant_id=consultant_id)
    consultant_images = list(ConsultantImages.objects.filter(consultant_id=consultant_id))
    fav_consultant_ids = []
    ...
    return render(request, 'single-consultant-details.html', {'consultant': consultant, 'consultant_details': consultant_details, 'random_image': random_image,'fav_consultant_ids': fav_consultant_ids})
...
def singel_consultant_review(request, consultant_id):
    ...
    reviews = Review.objects.filter(consultant=consultant_id).order_by('-created_at')[:10]
    ...
    for review in reviews:
        ...
        student_details = StudentDetails.objects.filter(dets_regs_id=review.student).first()
        ...
        review_data = {
            ...
            'image_url': student_details.student_image.url if student_details and student_details.student_image else None,
    ...
    if not request.user.is_authenticated:
        ...
        if 'last_activity_time' in request.session:
            ...
            if (current_time - last_activity_time).total_seconds() > 300:  # 5 minutes = 300 seconds
...
def save_review(request):
    if request.method == 'POST':
        ...
        if request.user.is_authenticated:
                ...
                if cap_json.get('success', False):
                    if student and consultant:
                        if raw_rating:
                            ...
                            print('number of review by the student: ', len(maximum_review))
                            if student_rating is None:
                                if len(maximum_review) < 5:
                                    ...
                                    return JsonResponse({'error': 'You can review at most 5 consultants' })
                        ...
                        return JsonResponse({'wrong_user_error': 'Not permitted to post a review. You need to be a student to post a review.'})
            ...
            return JsonResponse({'no_user_error': 'You need to login as a student to post a review'})
...
def feedback(request, consultant_id):
    ...
    consultant_details = get_object_or_404(ConsultantDetails, consultant_id=consultant.id)
...
def save_feedback(request):
    if request.method == 'POST':
        ...
        phone = request.POST.get('phone')
        subject = request.POST.get('subject')
        country_ids = request.POST.getlist('country')  # Extract multiple selected countries as a list
        ...
        if cap_json.get('success', False):
            ...
            if name and email and phone and subject and country_ids and message:
                ...
                return JsonResponse({'error': 'Please fill up all the required fields'}, status=400)
        else:
            # reCAPTCHA verification failed
            return JsonResponse({'error': 'reCAPTCHA verification failed. Please try again.'}, status=400)
    else:
...
def consaltant_wise_scholarship_singel_page(request, scholarship_id):
    ...
    if not scholarship:
        ...
        consultant_wise_scholarship = get_object_or_404(ConsultantWises, scow_id=scholarship_id)
...
def country_wise_scholarship(request):
    ...
    return render(request, 'country_wise_scholarship.html', {'country_wise': country_wise,'page_name': page_name,'page_description': page_description,
         'page_keywords': page_keywords})
@login_required(login_url='login_student')
def country_wise_scholarship_single(request, scw_id):
    ...
    page_description = "This is a top level student visa related information web portal, you can get any types of information from here. also you can take lates of visa agent information from here."
    page_keywords = "education visa consultant agent in dhaka, student visa informatin agent, student visa need, student visa consultant company, need student visa from dhaka,"
    return render(request, 'country_wise_scholarship_single.html', {'scholarship': scholarship,'page_name': page_name,         'page_description': page_description,
         'page_keywords': page_keywords})
...
def add_to_favorite(request, consultant_id):
    ...
    messages.success(request, f'{consultant.full_name} added to favorites successfully.')
        ...
        consultant_list = [
            {
                ...
                'total_reviews': Review.objects.filter(consultant=consultant.id).count()  # Count total reviews for the consultant
            }
...
def save_secondary_result_information(request):
    if request.method == 'POST':
        ...
        if existing_result and student.status == 2:
            return JsonResponse({'error': f"Your Secondary result has already been verified. You can't change your secondary result."})

        else:
            print('entered to save secondary result')
            if secondary and secondary_board and secondary_result and secondary_roll_no and secondary_reg_no and secondary_certificate_no and secondary_passing_year:
                image_size = 1024 * 1024
                if secondary_certificate_copy and secondary_certificate_copy.size > image_size:
                    return JsonResponse({'error': 'Image size of Secondary Education Certificate exeeds 1MB'})
                ...
                if existing_result is not None:
                    if not existing_result.secondary_certificate_copy:
                        if not secondary_certificate_copy or secondary_certificate_copy is None:
                            return JsonResponse({'error': 'Please provide Secondary Equivalent Certificate images'})
                ...
                return JsonResponse({'error': 'Please fill up the details in Secondary section'})
...
def save_higher_result_information(request):
    if request.method == 'POST':
        ...
        if existing_result and student.status == 2:
            return JsonResponse({'error': f"Your Higher Secondary result has already been verified. You can't change your higher secondary result."})
        
        else:
            if higher and higher_board and higher_result and higher_roll_no and higher_reg_no and higher_certificate_no and higher_passing_year:
                image_size = 1024 * 1024
                if higher_certificate_copy and higher_certificate_copy.size > image_size:
                    return JsonResponse({'error': 'Image size of Secondary Education Certificate exeeds 1MB'})
                ...
                if existing_result is not None:
                    if not existing_result.higher_certificate_copy:
                        if not higher_certificate_copy or higher_certificate_copy is None:
                ...
                if diploma_result or diploma_roll_no or diploma_reg_no or diploma_certificate_no:
                    print('got diploma result')
                    if diploma_board and diploma_result and diploma_roll_no and diploma_reg_no and diploma_certificate_no and diploma_passing_year:
                        image_size = 1024 * 1024
                        if diploma_certificate_copy and diploma_certificate_copy.size > image_size:
                            return JsonResponse({'error': 'Image size of Diploma Education Certificate exeeds 1MB'})

                        if existing_result is not None:
                            if not existing_result.secondary_certificate_copy:
                                if not higher_certificate_copy or higher_certificate_copy is None:
                                    return JsonResponse({'error': 'Please provide both Diploma Certificate images'})
                        ...
                        return JsonResponse({'error': 'Please fill up the details in Diploma Education section'})
                ...
                return JsonResponse({'error': 'Please fill up the details in Higher Education section'})
...
def save_undergraduation_result_information(request):
    if request.method == 'POST':
        ...
        if existing_result and student.status == 2:
            return JsonResponse({'error': f"Your Undergraduation result has already been verified. You can't change your undergraduation result."})
            ...
            if undergraduation_result and university_name:
                ...
                return JsonResponse({'error': 'University name and Current CGPA is required'})
    ...
    if request.method == 'POST':
        ...
        if student.status != 2:
            ...
            for group_number, data in enumerate(formatted_data):
                ...
                certificate_copy_str = f"masters_certificate_copy_{group_number}"
                certificate_copy = request.FILES.get(certificate_copy_str, '')
                masters_degree_check = MastersDegree.objects.filter(student_id=student_id).first()
                ...
                if (result != 0.0) and (university is not None or university != 'null'):
                    ...
                    if certificate_copy != '':
                        ...
                        masters_degree.certificate_copy = certificate_copy
                        masters_degree.certificate_copy.name = certificate_file_path(masters_degree, certificate_copy.name, 'ungraduation')
            ...
            return JsonResponse({'success': True, 'success_message': 'Only the unverified Masters Degrees Have been saved.'})
    
        else:
            return JsonResponse({'error': f"Your Masters result has already been verified. You can't change your masters result."})
...
def save_phd_result_information(request):
    if request.method == 'POST':
        ...
        #         return JsonResponse({'error': 'Image size of Higher Education Certificate exeeds 1MB'})
        if existing_result and student.status == 2:
            return JsonResponse({'error': f"Your PHD result has verified. You can't change your PHD result."})
    ...
    if request.method == 'POST':
        ...
        if student.status != 2:
            ...
            for group_number, data in enumerate(formatted_course_data):
                ...
                other_courses_check = OtherCertification.objects.filter(student_id=student_id).first()
                    ...
                    if other_certification is not None:
                        if not other_certification.certificate_copy:
                            if not certificate_copy or certificate_copy is None:
                                return JsonResponse({'error': 'Please provide Academic Certificate images'})
            ...
            return JsonResponse({'error': f"Your Extra Courses result has already been verified. You can't change your this result."})
...
def get_exam_type(request):
    ...
    return JsonResponse({'success': True, 'board_names': exam_board_list, 'board_type': board_type})
...
def student_profile(request):
    ...
    try:
        ...
        masters_degrees = MastersDegree.objects.filter(student_id=student_user).order_by('-passing_year')[:2]
        other_certifications = OtherCertification.objects.filter(student_id=student_user)
        ...
        student_details = StudentDetails.objects.filter(dets_regs_id=student_user).first()
...
def edit_student_profile(request):
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            student_details, created_at = StudentDetails.objects.get_or_create(dets_regs_id=student_user, dets_status=1)
        ...
        return render(
            ...
            {'student': student, 'student_details': student_details, 'results': results, 'password_form': password_form, 'thanas': thanas, 'districts': districts,'page_name':page_name,         'page_description': page_description,
         'page_keywords': page_keywords}
        )
...
def login_admin(request):
    if request.method == 'POST':
        ...
        if identifier is not None and password is not None:
            # Check if the identifier is an email or phone number
            if '@' in identifier:  # Assuming email contains '@'
                user = EmailBackend().authenticate(request, email=identifier, password=password,user_type=0)
            else:
                user = EmailBackend().authenticate(request, phone=identifier, password=password,user_type=0)
...
def address_view(request):
    ...
    if root_user:
        ...
        if last_address:
            return render(request, 'address.html', {'addresses': [last_address], 'page_name': page_name, 'page_description': page_description, 'page_keywords': page_keywords})
        else:
            # If no matching address found, handle this case as per your requirement
            return render(request, 'error.html', {'message': 'No matching address found'})
    else:
...
def map_view(request):
    ...
    return render(request, 'map.html', {'consultant_maps': consultant_maps,'page_name': page_name,'page_description':page_description,'page_keywords':page_keywords})
