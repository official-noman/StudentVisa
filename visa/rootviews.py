        ...
        def _wrapped_view(request, *args, **kwargs):
            ...
            try:
                ...
                if user.user_role == 2:
                    ...
                    return permission_required(permission, raise_exception=True)(view_func)(request, *args, **kwargs)
                else:
...
def root_home(request):
    ...
    total_credit = Balances.objects.filter(acc_credit__isnull=False).aggregate(total_credit=Sum('acc_credit'))['total_credit'] or 0

    # Calculate total debit amount
    total_debit = Balances.objects.filter(acc_debit__isnull=False).aggregate(total_debit=Sum('acc_debit'))['total_debit'] or 0
...
def export_dashboard_data(request):
    if request.method == 'POST':
        ...
        return JsonResponse({'error': 'Invalid request method. Must be POST.'}, status=400)
...
def root_consultant_list_json(request):
    ...
    root_consultants = Users.objects.filter(
        user_role=5,
        active_status__in=[None, 0]
    )

    # Extract specific fields from the queryset including 'phone' and 'id'
    root_consultants_data = root_consultants.values('id', 'full_name', 'email', 'company_name', 'phone', 'active_status')
    ...
    return JsonResponse({'root_consultants': root_consultants_list, 'total_count': total_count}, safe=False)
...
def create_reply(request, message_id):
    if request.method == 'POST':
        ...
        try:
            ...
            return JsonResponse({'status': 'success', 'reply_id': reply.id})  # Return reply ID
        ...
        if request.method == 'POST':
            ...
            if instance:
                ...
                if image:
                    ...
                    if instance.image:
                        old_image_path = os.path.join(settings.MEDIA_ROOT, instance.image.name)
...
@login_required
@root_required
@permission_required('visa.add_offerletters', 'visa.view_offerletters', raise_exception=True)
...
@login_required
@root_required
@permission_required(('visa.add_scholarships', 'visa.change_scholarships'), raise_exception=True)
def consultant_wise_scholarship(request):
    ...
    if request.method == 'POST':
        ...
        country_id = request.POST.get('country_id')  # Get the selected country ID from the form

        consultant_instance = Users.objects.get(id=consultant_id)
        country_instance = Countries.objects.get(country_id=country_id)
        expire_time_str = request.POST.get('expiration_time')  # Get the selected expiration time from the form as a string

        # Convert the string to a datetime object
        expire_time = datetime.strptime(expire_time_str, '%Y-%m-%d') if expire_time_str else None# Retrieve the country instance
    ...
    return render(request, 'roottemplates/consultant_wise_scholarship.html', {'instances': instances, 'consultants': consultants, 'countries': countries})
...
def country_wise_scholarship(request):
    ...
    if request.method == 'POST':
        ...
        if scw_country_id:
            country_wise_check = CountryWises.objects.filter(scw_country_id=scw_country_id).first()

            if country_wise_check is not None:
                print('error message printed')
                messages.error(request, f"The scholarship for this country already exists. Please go to edit to update the information")
                return render(request, 'roottemplates/country_wise_scholarship.html', {'country': countries})
...
def create_root(request):
    if request.method == 'POST':
        ...
        try:
            ...
            if Users.objects.filter(email=email).exists() or CustomUser.objects.filter(email=email).exists():
                messages.error(request, 'Email already exists')
            elif Users.objects.filter(phone=phone).exists() or CustomUser.objects.filter(phone=phone).exists():
...
def grant_permission(request, user_id):
    if request.method == 'POST':
        try:
            ...
            group_permissions = {
                'Log Entry': ['auth.add_logentry', 'auth.change_logentry', 'auth.delete_logentry', 'auth.view_logentry'],
                'Permissions': ['auth.add_permission', 'auth.change_permission', 'auth.delete_permission', 'auth.view_permission'],
                'Accounts': ['visa.add_balances', 'visa.change_balances', 'visa.delete_balances', 'visa.view_balances','visa.add_rates','visa.change_rates','visa.delete_rates',
                'visa.view_rates'],
                'Consultant': ['visa.add_users', 'visa.change_users', 'visa.delete_users', 'visa.view_users'],
                'Message': ['visa.add_message', 'visa.change_message', 'visa.delete_message', 'visa.view_message'],
                'Apply Process': ['visa.add_offerletters', 'visa.change_offerletters', 'visa.delete_offerletters', 'visa.view_offerletters'],
                'Rate': ['visa.add_rates', 'visa.change_rates', 'visa.delete_rates', 'visa.view_rates'],
               
                'Scholarship': ['visa.add_scholarships', 'visa.change_scholarships', 'visa.delete_scholarships', 'visa.view_scholarships'],
                'Students': ['visa.add_students', 'visa.change_students', 'visa.delete_students', 'visa.view_students'],
            }

            # Get the current permissions of the user
            current_permissions = user_instance.user_permissions.values_list('codename', flat=True)
            ...
            for permission_codename in permissions_to_remove:
                try:
                    ...
                    if codename_without_prefix in current_permissions:
                        permission = Permission.objects.get(codename=codename_without_prefix)
            ...
            existing_permissions = user_instance.user_permissions.values_list('codename', flat=True)

            # Match group names with existing permissions and toggle checked status
            matched_group_names = []
            for group_name, permissions in group_permissions.items():
                if any(permission.split('.')[-1] in existing_permissions for permission in permissions):
                    matched_group_names.append(group_name)

            # Check if any permissions were granted
            if permissions_to_add:
                return JsonResponse({'message': 'Permissions updated successfully.', 'matched_group_names': matched_group_names})
            else:
...
def activeconsultant(request):
    active_consultants = Users.objects.filter(user_role=5,active_status__in=[1, 3, 4, 5]).select_related('consultant_user')
    ...
    consultant_data_sorted = sorted(consultant_data, key=lambda x: (x['full_name'], x['company_name']))
...
def suspend_list(request):
    ...
    suspended_users = Users.objects.filter(active_status=2).order_by('company_name')  # Assuming 2 represents suspended status, adjust as needed
...
def make_suspend(request, student_id):
    try:
        ...
        student.status = 3  # Update status to suspended (assuming 3 represents suspended status)
        student.save()
        return JsonResponse({'success': True, 'message': f"Student {student.full_name}'s account has been suspended."})
...
def active_student_details(request, student_id):
    try:
        ...
        all_verified = all(entry.get('verification_status') == 'verified' for entry in education_data)
...
def update_verification_status(request):
    if request.method == 'GET':
        ...
        course_names_list = [certification.course_name.name for certification in course_names]
        ...
        if verification_status not in valid_statuses:
            return JsonResponse({'error': 'Invalid verification status provided.'}, status=400)

        # Determine the verification status field based on the verification type
        if verification_type == 'SSC' or verification_type == 'O level' or verification_type == 'Dhakhil':
            result.secondary_verification_status = verification_status
            result.secondary_rejection_note = rejection_note
            result.save()

        elif verification_type == 'HSC' or verification_type == 'A level' or verification_type == 'Vocational' or verification_type == 'Diploma':
            ...
            masters_degree.save()
            print('verification_status in masters courses after saving: ', masters_degree.verification_status)
            ...
            return JsonResponse({'error': 'Invalid verification type provided.'}, status=400)
        
        return JsonResponse({'success': True, 'message': f'{verification_type.capitalize()} verification status updated to {verification_status}.'})
...
def verify_student_account(request, student_id):
    ...
    if student_details.exists():
        ...
        consultant_id = request.user.id  # Assuming consultant_id comes from the request user
        consultant_status, created = ConsultantStatus.objects.get_or_create(student=student_details, consultant_id=consultant_id)
...
def get_consultant_balance(request):
    if request.method == 'GET':
        ...
        try:
            total_credit = Balances.objects.filter(acc_pay_to=consultant_id).aggregate(total_credit=Sum('acc_credit'))['total_credit'] or 0
            total_debit = Balances.objects.filter(acc_paid_by=consultant_id).aggregate(total_debit=Sum('acc_debit'))['total_debit'] or 0
...
def get_last_transactions(request):
    if request.method == 'GET':
        ...
        try:
            ...
            transaction_list = [{
                'date': transaction.created_at.strftime('%Y-%m-%d %H:%M:%S'),  # Format date as string
                'reference': transaction.acc_pay_ref,
                'amount': transaction.acc_credit if transaction.acc_credit else -transaction.acc_debit,  # Use credit if available, else use debit
                'type': 'Credit' if transaction.acc_credit else 'Debit',  # Identify transaction type
...
def add_address(request):
    ...
    if request.user.user_type == 0:
        ...
        if request.method == 'POST':
            ...
            phone = request.POST.get('phone')
            email = request.POST.get('email')
            map_location = request.POST.get('map_location')  # Assuming you have a form field for map location
        ...
        return render(request, 'roottemplates/add_address.html', {'existing_address': existing_address, 'existing_map': existing_map})
    ...
    if request.method == 'POST':
        ...
        if instance:  # Update operation
            ...
            if exp_img:
                ...
                if instance.exp_img:
                    existing_img_path = os.path.join(settings.MEDIA_ROOT, instance.exp_img.name)
...
def add_client(request):
    if request.method == "POST":
        ...
        try:
            ...
            return redirect('client_list')  # Redirect to the client list page after adding client
        except Exception as e:
...
def save_benefit_for_consultant(request):
    ...
    return render(request, 'roottemplates/benefit.html', {'benefit_list': instance_benefit})
...
def get_all_consultants(request):
    try:
        ...
        for consultant in consultants:
            ...
            consultant_credits = Balances.objects.filter(acc_pay_to=consultant.id).aggregate(Sum('acc_credit'))['acc_credit__sum'] or 0.0
            consultant_debits = Balances.objects.filter(acc_paid_by=consultant.id).aggregate(Sum('acc_debit'))['acc_debit__sum'] or 0.0
 
            consultant_balance = consultant_credits - consultant_debits
            consultant_details = ConsultantDetails.objects.filter(consultant_id=consultant.id).first()
            consultant_data.append({
                ...
                'phone': consultant.phone,
                'company_name': consultant.company_name,
                'establishment_date': consultant.est_date.strftime('%B %d, %Y') if consultant.est_date else None,
                'balance': consultant_balance,
                'experience': consultant_details.experience if consultant_details else None,
                'consultant_img': consultant_details.consultant_img.url if consultant_details and consultant_details.consultant_img else None,
                # Add other fields as needed
            })
...
def balance_list(request):
    ...
    return render(request, 'roottemplates/balance_list.html', {'credit_transactions': credit_transactions, 'debit_transactions': debit_transactions})
...
def consultant_wise_scholarship_list(request):
    ...
    return render(request, 'roottemplates/consultant_wise_scholarship_list.html', {'instances': instances})
...
def edit_consultant_wise_scholarship(request, scow_id):
    ...
    countries = Countries.objects.all().values('country_id', 'country_name')  # Adjusted to use 'country_id'
    if request.method == 'POST':
        scow_text = request.POST.get('scow_text')
        scow_whocanapply = request.POST.get('scow_whocanapply')
        scow_status = request.POST.get('scow_status')
        ...
        country_instance = get_object_or_404(Countries, country_id=country_id)  # Adjusted to use 'country_id'
        ...
        data = {
            ...
            'country_id': instance.country_name.country_id if instance.country_name else None,  # Send country ID instead of country name
...
def delete_consultant_wise_scholarship(request, scow_id):
    ...
    return JsonResponse({'success': True, 'message': 'Scholarship deleted successfully', 'scow_id': scow_id})
...
def view_scholarship(request, scholarship_id):
    if request.method == 'GET':
        ...
        scholarship_data = {
            ...
            'updated_at': scholarship.updated_at.strftime('%Y-%m-%d %H:%M:%S') if scholarship.updated_at else None
        }
...
def delete_scholarship_uploaded(request, scholarship_id):
    ...
    return JsonResponse({'success': True, 'message': 'Scholarship deleted successfully', 'scholarship_id': scholarship_id})
...
def edit_course(request, course_id):
    ...
    if request.method == 'POST':
        ...
        if new_course_name:
            ...
            return JsonResponse({'success': False, 'message': 'New course name cannot be empty'})
    else:
